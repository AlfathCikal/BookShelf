document.addEventListener('DOMContentLoaded', () =>{
	...
	function updateProgressStyle (){
		...
		if (window.innerWidth <= 860){
			...
			document.querySelector('.progressSlider .left-slide').classList.add('fa-arrow-left');
			document.querySelector('.progressSlider .left-slide').classList.remove('fa-arrow-up');
			
			// right arrow or down arrow
			document.querySelector('.progressSlider .right-slide').classList.add('fa-arrow-right');
			document.querySelector('.progressSlider .right-slide').classList.remove('fa-arrow-down');
		} else {
			progressSlider.classList.add('upDown');
			// right arrow or down arrow
			document.querySelector('.progressSlider .left-slide').classList.add('fa-arrow-up');
			document.querySelector('.progressSlider .left-slide').classList.remove('fa-arrow-left');
			
			document.querySelector('.progressSlider .right-slide').classList.add('fa-arrow-down');
			document.querySelector('.progressSlider .right-slide').classList.remove('fa-arrow-right');
		};
	}
...
function createProgressCard(bookObject){
	...
	progressCard.innerHTML = `
		<img class="progress-img" src="assets/img/HarryPotterBookCover.jpg" alt="book cover">
...
function searchBook(){
	...
	if (booksFilter.length === 0){
		...
		document.querySelector('.uncompletedSlider .right-slide').dispatchEvent(new Event('click'));
		document.querySelector('.completedSlider .right-slide').dispatchEvent(new Event('click'));
		document.querySelector('.progressSlider .right-slide').dispatchEvent(new Event('click'));
	}
...
function createBook(bookObject){
	...
	book.innerHTML = `<img class="book-cover-img" src="assets/img/HarryPotterBookCover.jpg" alt="book cover">`;
...
function initSlider (n, nodeList, slideIndex){
	...
	if (slideIndex === 'progressIndex'){
		if (nodeList.classList.contains('upDown')){
			nodeList.style.transform = `translateY(${-7.5 * (indexing[slideIndex] - 1)}rem)`;
		} else {
			nodeList.style.transform = `translateX(${-19.5 * (indexing[slideIndex] - 1)}rem)`;
		}
	} else {
		nodeList.style.transform = `translateX(${-24 * (indexing[slideIndex] - 1)}rem)`;
	}
}
...
document.querySelector('.uncompletedSlider .left-slide').addEventListener("click", () => plusSlide(-1, document.querySelector('#uncompletedBooks'), 'uncompletedIndex'));
document.querySelector('.uncompletedSlider .right-slide').addEventListener("click", () => plusSlide(1, document.querySelector('#uncompletedBooks'), 'uncompletedIndex'));

// complitedSlide button
document.querySelector('.completedSlider .left-slide').addEventListener("click", () => plusSlide(-1, document.querySelector('#completedBooks'), 'completedIndex'));
document.querySelector('.completedSlider .right-slide').addEventListener("click", () => plusSlide(1, document.querySelector('#completedBooks'), 'completedIndex'));

// ProgressSlide button
document.querySelector('.progressSlider .left-slide').addEventListener("click", () => plusSlide(-1, document.querySelector('.progressBookList'), 'progressIndex'));
document.querySelector('.progressSlider .right-slide').addEventListener("click", () => plusSlide(1, document.querySelector('.progressBookList'), 'progressIndex'));
...
document.querySelector('#editModal .edit-close-btn').addEventListener('click', ()=> closeModal())
